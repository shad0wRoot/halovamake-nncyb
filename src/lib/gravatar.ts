...
function md51(text: string) {
  ...
  for (; index + 64 <= text.length; index += 64)
    md5cycle(state, md5blk(text.substring(index, index + 64)))

  const tail = new Array<number>(16).fill(0)
  const remaining = text.substring(index)

  for (index = 0; index < remaining.length; index++)
    tail[index >> 2] |= remaining.charCodeAt(index) << ((index % 4) << 3)
...
function md5blk(text: string) {
  const blocks: number[] = []
  for (let index = 0; index < 64; index += 4) {
    blocks[index >> 2] = text.charCodeAt(index)
...
function rhex(value: number) {
  ...
  for (let index = 0; index < 4; index++)
    output += HEX_CHARS[(value >> (index * 8 + 4)) & 0x0F] + HEX_CHARS[(value >> (index * 8)) & 0x0F]
  return output
}
...
export function getGravatarUrl(email: string, size = 64) {
  ...
  return `https://www.gravatar.com/avatar/${md5(normalizedEmail)}?s=${size}&d=identicon&r=g`
}
